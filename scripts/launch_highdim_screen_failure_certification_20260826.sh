#!/usr/bin/env bash
# Deterministic reproduction of the 4 screen rows that failed before round 0
# (closeout plan SS6.2). --keep-going: a genuinely different failure on any
# one row must not block certifying the others.
set -euo pipefail

repo_root="/home/arnav22103/FederatedDeepGMM"
python_bin="/home/arnav22103/miniconda3/envs/fedgmm/bin/python"
campaign="experiments/highdim_coauthor_protocol_v1/screen_failure_certification_20260826"
...
echo "SCREEN FAILURE CERTIFICATION RUNS COMPLETE. Review outcomes against SS6.2's rule:"
