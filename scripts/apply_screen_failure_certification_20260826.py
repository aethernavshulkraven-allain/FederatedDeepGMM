...
failed before federated round 0 to its independent reproduction (closeout
plan SS6.2).

Never writes into the original screen row directories: PROTOCOL_DECISION_
ADDENDUM_20260826.md requires terminal-pretraining evidence to be "written
by the training process itself at the moment of failure." The original
screen attempt's process only ever wrote effective_config.json before
failing -- it never had the chance to write a pretraining_failure.json, and
no later script can honestly claim to be that process. The reproduction run
(in a separate, dedicated namespace) is a *different* process invocation
that really did write its own real pretraining_failure.json; this script
only records the link between the two run_ids, for
score_highdim_screen_post_bn_20260822.py and check_manifest_stage_complete.py
to resolve via run_manifest.resolve_certified_run(). The original screen
directories are read-only here and stay exactly as the original process
left them.
"""
