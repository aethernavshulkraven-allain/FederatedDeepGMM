    ...
    # Conditional batch normalization
        if self.training:
         if data.size(0) > 1:
            ...
            # Apply batch normalization using running stats when batch size is 1
            data = F.batch_norm(
                data, self.bn.running_mean, self.bn.running_var,
                weight=self.bn.weight, bias=self.bn.bias, training=False)
        else:
        # Use stored running stats for evaluation
          data = self.bn(data)
        return self.linear_3(data)
...
class CIFAR10CNN(LeakySoftmaxCNN):
    def __init__(self, cuda):
        LeakySoftmaxCNN.__init__(
            self, input_c=3, input_h=32, input_w=32, channel_sizes=[32, 64],
            kernel_sizes=[3, 3], extra_padding=[0, 1],
            activation=nn.LeakyReLU, cuda=cuda)
    def forward(self, data):
        data = data.view(data.shape[0], 3, 32, 32)
